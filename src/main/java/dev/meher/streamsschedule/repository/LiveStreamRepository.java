package dev.meher.streamsschedule.repository;
...
public class LiveStreamRepository {
    ...
    public LiveStreamRepository() {
        streams.add(new LiveStream(UUID.randomUUID().toString(), "Immutable stream", "Created an Immutable stream", "", LocalDateTime.of(2022, 8, 3, 15, 30, 0), LocalDateTime.of(2022, 8, 3, 16, 30, 0)));
    }
    ...
    public void update(LiveStream stream, String id) {
        LiveStream existing = streams.stream().filter(s->s.id().equals(id)).findFirst().orElseThrow(()->new IllegalArgumentException("Stream not found Exception"));
